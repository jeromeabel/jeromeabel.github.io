...
| [ ]  | R3.3      | **pass 1 clean** on the 19:20 export — verify 0/0/0/0, verify-raw 1050 new (all baselined kinds + SVG art) / 36 accepted / **0 stale** (15 pruned, each resolved against the fresh graph), responsive exact, 57/57. `prose-link-annotation` allowlisted ×2 with reasons. **Re-run owed** — R3.6-prep wrote to the Figma file, so this snapshot is dead; needs a fresh `File > Export`                                                           |
