...
| [ ]  | R3.3   | **pass 1 clean** on the 19:20 export — verify 0/0/0/0, verify-raw 1050 new (all baselined kinds + SVG art) / 36 accepted / **0 stale** (15 pruned, each resolved against the fresh graph), responsive exact, 57/57. `prose-link-annotation` allowlisted ×2 with reasons. **Re-run owed** — the defect repair invalidates this snapshot      |
| [x]  | R3.4   | `figma-ds-file.md` rewritten to final state — 101→362 lines; roster 46, 9 pages, 32-frame grid, 407 collections; new **⚠️ Open defects** section above the token tables                                                                                                                                                                     |
