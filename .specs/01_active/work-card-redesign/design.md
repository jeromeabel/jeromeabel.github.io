...
Editorial doctrine already states the difference (`editorial-v4/design.md` §1): _Work =
nouns with outcomes; Blog = verbs with reasoning. Work entries are covers, articles are
chapters._ For this portfolio specifically: blog posts are **deep and recent**, and they
...
Today both in-scope surfaces render the same three entries — Home strip and `/work` Selected
both call `getFeaturedWorks()` with no count. **Target state: Home shows 3 featured, `/work`
Selected shows 4** — `/work` is a superset (the Home three plus one more). That means
`getFeaturedWorks` grows a count (or the callers slice), and a fourth entry must join
`featured` — which entry is an editorial decision the design round proposes, not decides
(`editorial-v4` §5: list reopened, only Malinette fixed).
...
- **Border** — 1px, `--color-border`, structure only. A full border means _aggregate
  entity_. Preview cards are borderless; the image is the frame. `border-2` is dead. Dashed
...
- **Hover** — one verb per surface, ≤150ms. Borderless preview card = title underline + slow
  cover scale, _coupled_. Hover must never dim or tint a cover, never turn a title accent,
...
show _what it is_ (discipline) and _when_ (year), then get out of the way. The current work
plate's two-line description is the main thing making it look like a blog card.

### Live-site survey (2026-08-15)

[survey.md](./survey.md) — ~25 live portfolios classified + layout literature. Headlines:
**1+N asymmetric appeared zero times**; the field splits into uniform case-block stacks and
text ledgers (this portfolio's Selected + `ArchiveTable` page structure mirrors exactly
that); the **uniform stack is modal at exactly 4 described projects** (brittanychiang,
kysondana, davidhellmann); grids are favored only at 6+ thumbnail-friendly entries;
numbering `01/02` is essentially absent in the wild despite 6 refs in the inspiration set;
descriptions at N≤5 are ≤2 sentences or cut entirely; nothing essential is hover-revealed
anywhere.
...
external-artifact affordance (`↗ Live · Repo`). _Pro:_ cheapest, and honest receipts —
these are the fields blog can never have. _Con:_ same silhouette as a blog row; distinction
is read, not seen.

**B · Numbered catalogue.** Mono `01 / 02 / 03` plus a hairline above each plate; caption
becomes `title` + `kicker`. _Pro:_ index vs feed is the clearest structural split; the
`featured` field already is a rank; mono numerals are the machine register the DS reserves
for generated facts. _Con:_ numbering implies a ranking that must be defensible; needs the
`getFeaturedWorks` sort fixed.

**C · Shape flip.** Work covers take a different ratio from blog's 16:9 — square, or 4:5.
_Pro:_ pre-attentive distinction, no extra text at all; objects and installations read
better square than letterboxed. _Con:_ re-crops across 19 previews; two ratio systems to
...
_Pro:_ parallel page structure with genuinely different content; matches the content model
and the Work-as-receipts doctrine. _Con:_ two big blocks compete on Home, and the writing
section is meant to lead in v3; description quality varies across entries. **Deeper con:**
the 1+N silhouette is blog's structural signature — even on `/work`, where no blog section
sits adjacent, it risks reading as "the blog layout again". The non-asymmetric form —
**equal-weight case blocks stacked**, all four at the same size — keeps D's content fit
without borrowing blog's hierarchy grammar.

**E · Spec-sheet card.** Cover plus a small mono key/value block (Year / Type / Stack /
Links). _Pro:_ maximum artifact-receipt reading, zero overlap with blog, reuses
`ArchiveTable`'s grammar in card form. _Con:_ dense; risks reading as a table fragment;
heavy mono budget on a page that already has mono dates.

**F · Overlay label.** Discipline + year set on the cover, title below. _Pro:_ gallery feel,
fits the art history. _Con:_ AA is unreliable across 19 heterogeneous covers; and it must be
...
by freshness is a time signal, and time belongs to blog. Home then reads: _blog = a feature
plus a feed; work = a numbered row of catalogue entries._ **D is held for `/work` Selected**
— where a case block has room and recruiters, not readers, are the audience — but explored
in **both forms**: 1+3 asymmetric and uniform equal-weight stack. The
[survey](./survey.md) settles the default: **uniform stack is the evidence-backed form** —
1+N was observed on zero of ~25 live sites, while the uniform stack is the modal pattern at
exactly 4 described projects. 1+3 stays explorable only with a specific argument that beats
the stack; 2×2 is a long shot at N=4 (grids win at 6+ thumbnail-friendly entries).
...
1. 4–6 variants on a new `EXP / WorkCard` page in `Magnet-DS`, token-bound, Desktop 1280 +
   Mobile 390 × Light + Dark. Every variant proposes **both layouts: Home at 3 cards and
   `/work` Selected at 4 cards** — a variant that only works at one count is incomplete.
...
4. A short note on which existing components would change (`WorkPreviewSection`, the work
   ...
   whether `getFeaturedWorks` must move from date sort to `featured` sort, and how the 3/4
   count split is served (count param vs caller-side slice).

## Open questions

1. **Selected list size — counts decided, membership open.** Home shows 3, `/work` Selected
   shows 4 (superset). Still open per `editorial-v4` §5 (only "Malinette is featured" fixed):
   which entry takes the fourth slot. The design round proposes; the editorial loop decides.
