...
- Why: IBM Carbon frames its duration/easing choice around a physical metaphor — elements "speed up quickly and slow down smoothly, obeying the physics of a light-weight material" — and ties duration to distance traveled with "a non-linear duration scale to achieve better perceived consistency across all distances," plus two named motion moods (Productive vs. Expressive) that map to different duration/easing sets. Material Design 3 similarly names two easing families (Standard vs. Emphasized) and frames the newer "expressive"/physics-based motion system around making interactions "feel more alive, fluid, and natural."
- Evidence: IBM Carbon Design System — https://carbondesignsystem.com/elements/motion/overview/ (confirmed via search-indexed content)
- Evidence: Google Material Design 3 — https://m3.material.io/styles/motion/easing-and-duration/tokens-specs
- Confidence: consensus
- Notes: I could not render the full M3 or Carbon motion pages directly (both are heavily client-rendered SPAs that WebFetch could not extract past the title), so exact numeric values here come from indexed search snippets rather than a page I fetched in full — treat the qualitative pattern (named moods + metaphor-based rationale) as solid, the specific ms/curve numbers as lower-confidence.
