...
**Goal:** Replace the flat `docs/superpowers/{specs,plans}` files with a single git-tracked specs system under `.specs/` — lifecycle folders, a dependency-free bash helper, and a generated INDEX dashboard.
...
- `specs.sh` resolves its own directory (`.specs/`) so it works from any CWD.
...
### Task 1: Scaffold `.specs/` skeleton + conventions file

Creates the folder structure, the conventions `CLAUDE.md`, and `.gitkeep` placeholders so the empty lifecycle folders are tracked. `.specs/` already exists and holds one legacy file (`2026-05-07-api-endpoints-blog-and-playground-design.md`) — leave it untouched here; Task 6 migrates it.

**Files:**

- Create: `.specs/CLAUDE.md`
- Create: `.specs/00_backlog/.gitkeep`
- Create: `.specs/01_active/.gitkeep`
- Create: `.specs/02_archives/.gitkeep`

**Interfaces:**

- Consumes: nothing.
- Produces: the directory layout `.specs/{00_backlog,01_active,02_archives}/` that Task 2's `specs.sh` scans, and `SPECS_DIR = .specs/`.

- [ ] **Step 1: Create the folders and placeholders**

```bash
cd /home/jabel/code/projects/jeromeabel.github.io
mkdir -p .specs/00_backlog .specs/01_active .specs/02_archives
touch .specs/00_backlog/.gitkeep .specs/01_active/.gitkeep .specs/02_archives/.gitkeep
```
...
```bash
git add .specs/CLAUDE.md .specs/00_backlog/.gitkeep .specs/01_active/.gitkeep .specs/02_archives/.gitkeep
git commit -m "feat(specs): scaffold .specs lifecycle folders + conventions"
```
...
- Create: `.specs/specs.sh`
- Create: `.specs/INDEX.md` (generated by running the script)

**Interfaces:**

- Consumes: the folder layout from Task 1.
- Produces: `specs.sh` with functions `die`, `valid_slug`, `artifacts`, `shipped_date`, `stamp_shipped`, `cmd_index`, and a `case` dispatcher. Later tasks add `cmd_new`, `cmd_activate`, `cmd_archive` and their dispatch cases. `cmd_index` writes `.specs/INDEX.md`.

- [ ] **Step 1: Write the failing smoke test**

Run (before the file exists): `bash .specs/specs.sh index`
Expected: FAIL with `bash: .specs/specs.sh: No such file or directory`.
...
```bash
chmod +x .specs/specs.sh
./.specs/specs.sh index
```

Expected: `Wrote /home/jabel/code/projects/jeromeabel.github.io/.specs/INDEX.md`

- [ ] **Step 4: Verify the generated INDEX**

Run: `cat .specs/INDEX.md`
Expected:
...
```bash
git add .specs/specs.sh .specs/INDEX.md
...
- Modify: `.specs/specs.sh` (add `cmd_new`, add `new` dispatch case)
...
Run: `./.specs/specs.sh new zz-smoke-test`
Expected: FAIL with `specs.sh: usage: specs.sh {new|activate|archive|index}` (exit 1) — `new` isn't dispatched yet.

- [ ] **Step 2: Add `cmd_new`**

In `.specs/specs.sh`, insert this function immediately **before** the `case "${1:-}"` block:
...
```bash
./.specs/specs.sh new zz-smoke-test "Smoke test"
cat .specs/00_backlog/zz-smoke-test.md
```
...
Run: `grep zz-smoke-test .specs/INDEX.md`
Expected: `- [zz-smoke-test](00_backlog/zz-smoke-test.md)`

- [ ] **Step 5: Verify the guardrails**

```bash
./.specs/specs.sh new zz-smoke-test 2>&1 || true    # duplicate
./.specs/specs.sh new "Bad Slug" 2>&1 || true        # invalid
```
...
```bash
rm .specs/00_backlog/zz-smoke-test.md
./.specs/specs.sh index
git add .specs/specs.sh .specs/INDEX.md
...
- Modify: `.specs/specs.sh` (add `cmd_activate`, add `activate` dispatch case)
...
Run: `./.specs/specs.sh activate zz-smoke-test`
Expected: FAIL with `specs.sh: usage: specs.sh {new|activate|archive|index}` (exit 1).

- [ ] **Step 2: Add `cmd_activate`**

In `.specs/specs.sh`, insert immediately **before** the `case "${1:-}"` block:
...
```bash
./.specs/specs.sh new zz-smoke-test "Smoke test"
git add .specs/00_backlog/zz-smoke-test.md .specs/INDEX.md
git commit -m "chore: temp smoke stub"
./.specs/specs.sh activate zz-smoke-test
```

Expected: prints `Activated zz-smoke-test` then `Wrote .../INDEX.md`.
Run: `test -f .specs/01_active/zz-smoke-test/spec.md && echo OK`
Expected: `OK`
Run: `grep zz-smoke-test .specs/INDEX.md`
...
```bash
git rm -r .specs/01_active/zz-smoke-test
./.specs/specs.sh index
git add .specs/specs.sh .specs/INDEX.md
...
- Modify: `.specs/specs.sh` (add `cmd_archive`, add `archive` dispatch case)
...
Run: `./.specs/specs.sh archive zz-smoke-test`
Expected: FAIL with `specs.sh: usage: specs.sh {new|activate|archive|index}` (exit 1).
...
./.specs/specs.sh archive zz-smoke-test
```

Expected final output: `Archived zz-smoke-test` then `Wrote .../INDEX.md`.
Run: `head -3 .specs/02_archives/zz-smoke-test/spec.md`
Expected (date = today):
...
```bash
git rm -r .specs/02_archives/zz-smoke-test
./.specs/specs.sh index
git add .specs/specs.sh .specs/INDEX.md
...
> **Note:** This task moves this plan file itself to `.specs/01_active/specs-system/plan.md` and its design to `.../design.md`. Remaining tasks (7, 8) do not reference the plan file, so the move is safe. After this task, the plan lives at its new path.
...
```bash
cd /home/jabel/code/projects/jeromeabel.github.io
mkdir -p .specs/01_active/seniority-update .specs/01_active/ui-refinements-v2 .specs/01_active/specs-system
mv docs/superpowers/specs/2026-07-17-seniority-update-design.md   .specs/01_active/seniority-update/design.md
mv docs/superpowers/plans/2026-07-17-seniority-update.md          .specs/01_active/seniority-update/plan.md
mv docs/superpowers/specs/2026-07-18-ui-refinements-v2-design.md  .specs/01_active/ui-refinements-v2/design.md
mv docs/superpowers/plans/2026-07-18-ui-refinements-v2.md         .specs/01_active/ui-refinements-v2/plan.md
mv docs/superpowers/specs/2026-07-18-specs-system-design.md       .specs/01_active/specs-system/design.md
mv docs/superpowers/plans/2026-07-18-specs-system.md             .specs/01_active/specs-system/plan.md
```

- [ ] **Step 2: Move the archived topics**

```bash
mkdir -p .specs/02_archives/home-writing-section .specs/02_archives/api-endpoints-blog-and-playground .specs/02_archives/site-refinements
mv docs/superpowers/specs/2026-07-18-home-writing-section-design.md   .specs/02_archives/home-writing-section/design.md
mv docs/superpowers/plans/2026-07-18-home-writing-section.md          .specs/02_archives/home-writing-section/plan.md
mv .specs/2026-05-07-api-endpoints-blog-and-playground-design.md  .specs/02_archives/api-endpoints-blog-and-playground/design.md
mv docs/plans/2026-05-08-api-endpoints-blog-and-playground.md         .specs/02_archives/api-endpoints-blog-and-playground/plan.md
mv docs/superpowers/specs/2026-07-18-site-critique-recommendations.md .specs/02_archives/site-refinements/design.md
mv docs/superpowers/plans/2026-07-18-site-refinements.md              .specs/02_archives/site-refinements/plan.md
```

- [ ] **Step 3: Move the backlog topics**

```bash
mkdir -p .specs/00_backlog/cv-system .specs/00_backlog/illustration-system .specs/00_backlog/ux-copy-review-findings
mv docs/superpowers/specs/2026-07-18-cv-system-design.md          .specs/00_backlog/cv-system/design.md
mv docs/superpowers/plans/2026-07-18-cv-system.md                .specs/00_backlog/cv-system/plan.md
mv docs/superpowers/specs/2026-07-18-illustration-system-design.md .specs/00_backlog/illustration-system/design.md
mv docs/superpowers/specs/2026-07-18-ux-copy-review-findings.md    .specs/00_backlog/ux-copy-review-findings/notes.md
```

- [ ] **Step 4: Remove emptied directories and placeholders**

```bash
rm -f .specs/00_backlog/.gitkeep .specs/01_active/.gitkeep .specs/02_archives/.gitkeep
...
Run: `find docs/superpowers docs/plans -type f 2>/dev/null; ls .specs`
Expected: the `find` prints nothing (both dirs gone); `ls .specs` shows `00_backlog 01_active 02_archives CLAUDE.md INDEX.md specs.sh`.
...
git commit -m "refactor(specs): migrate specs/plans into .specs lifecycle folders"
```
...
**Files:** creates `.specs/00_backlog/{flashless-dark-mode,home-animation-toggle,blog-toc,contact-images-animation,work-about-blog,blog-v2-1}.md`.
...
```bash
cd /home/jabel/code/projects/jeromeabel.github.io
./.specs/specs.sh new flashless-dark-mode "Flashless dark mode"
./.specs/specs.sh new home-animation-toggle "Home animation toggle"
./.specs/specs.sh new blog-toc "Blog table of contents"
./.specs/specs.sh new contact-images-animation "Contact images animation"
./.specs/specs.sh new work-about-blog "Work / About / Blog relationship"
./.specs/specs.sh new blog-v2-1 "Blog v2.1"
```
...
`.specs/00_backlog/flashless-dark-mode.md` body:
...
`.specs/00_backlog/home-animation-toggle.md` body:
...
`.specs/00_backlog/blog-toc.md` body:
...
`.specs/00_backlog/contact-images-animation.md` body:
...
`.specs/00_backlog/work-about-blog.md` body:
...
`.specs/00_backlog/blog-v2-1.md` body:
...
```bash
./.specs/specs.sh index
grep -A10 '## Backlog' .specs/INDEX.md
```
...
```bash
git add .specs/00_backlog .specs/INDEX.md
...
Adds a short "Specs & Planning" section to the project `CLAUDE.md` so the brainstorming and writing-plans skills write into `.specs/01_active/<slug>/`, and shipped work is archived via `specs.sh`.
...
- Consumes: the shipped specs system (`.specs/`, `specs.sh`).
