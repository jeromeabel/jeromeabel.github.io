...
const walk = (n) => {
  ...
  if (
    ["FRAME", "RECTANGLE"].includes(n.type) &&
    Array.isArray(n.fills) &&
    n.fills.some(isWhite) &&
    !(bv.fills && bv.fills.length)
  )
    whiteFills.push({ id: n.id, name: n.name });
  if (
    n.parent &&
    "clipsContent" in n.parent === false &&
    n.parent.type === "FRAME"
  ) {
    const p = n.parent;
    if (
      n.x < -1 ||
      n.y < -1 ||
      n.x + n.width > p.width + 1 ||
      n.y + n.height > p.height + 1
    )
