...
and satisfy R4's _intent_ (predictable per-topic structure, ≤2 disclosure levels) with a fixed
...
- Dissolving the Elements chapter means re-homing ~17 masters' specimens into the new Components
  ...
  task.
- "Icons" moving from Elements into Foundations: the Icon _component itself_ (the flat asset
  library, e.g. `arrow-right`) still needs a home in the flat Components list too, since it's
  instantiable — Foundations documents icon _sizing/usage rules_ (16/20/24), Components documents
